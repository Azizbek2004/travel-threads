...
export const getEvents = async (filter?: EventFilter): Promise<Event[]> => {
  try {
    ...
    if (filter) {
      ...
      if (filter.startDate) {
        eventQuery = query(eventQuery, where("startDate", ">=", filter.startDate))
      }
    ...
    if (filter?.location) {
      ...
      return events.filter((event) => event.location?.name.toLowerCase().includes(lowerLocation))
    }
...
export const getEventsByMonth = async (year: number, month: number): Promise<Event[]> => {
  try {
    ...
    const eventQuery = query(eventsCollection, where("startDate", ">=", startDate), where("startDate", "<=", endDate))
...
export const updateEvent = async (id: string, eventData: Partial<Event>): Promise<void> => {
  try {
    ...
    if (eventData.location?.lat && eventData.location?.lng) {
      const geopoint = new GeoPoint(eventData.location.lat, eventData.location.lng)
...
export const getUserEvents = async (userId: string): Promise<Event[]> => {
  try {
    const eventQuery = query(eventsCollection, where("authorId", "==", userId), orderBy("startDate", "asc"))
...
export const getAttendingEvents = async (userId: string): Promise<Event[]> => {
  try {
    ...
    throw new Error("Failed to fetch events you're attending. Please try again.")
  }
}
...
export const getInterestedEvents = async (userId: string): Promise<Event[]> => {
  try {
    ...
    throw new Error("Failed to fetch events you're interested in. Please try again.")
  }
}
...
export const getUpcomingEvents = async (limit = 10): Promise<Event[]> => {
  try {
    ...
    const eventQuery = query(eventsCollection, where("startDate", ">=", now), orderBy("startDate", "asc"), limit(limit))
...
export const getPopularEvents = async (limit = 10): Promise<Event[]> => {
  try {
    ...
    const eventQuery = query(eventsCollection, where("startDate", ">=", now), orderBy("startDate", "asc"))

    const snapshot = await getDocs(eventQuery)
    const events = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Event)

    // Sort by number of attendees
    const sortedEvents = events.sort((a, b) => (b.attendees?.length || 0) - (a.attendees?.length || 0))
