...
export const createPost = async (postData: Partial<Post>): Promise<string> => {
  ...
  if (postData.location?.lat && postData.location?.lng) {
    ...
    postData.geopoint = new GeoPoint(postData.location.lat, postData.location.lng)
...
// Comment functions
export const addComment = async (comment: Partial<Comment>): Promise<string> => {
...
export const getReplies = async (commentId: string): Promise<Comment[]> => {
  const q = query(commentsCollection, where("parentId", "==", commentId), orderBy("createdAt", "asc"))
...
export const getUserProfile = async (userId: string | undefined): Promise<UserProfile | null> => {
...
export const getFollowingPosts = async (userId: string): Promise<Post[]> => {
  ...
  const q = query(postsCollection, where("authorId", "in", following), orderBy("createdAt", "desc"), limit(50))
...
export const getOrCreateConversation = async (userIds: string[]): Promise<string> => {
...
export const addReactionToMessage = async (messageId: string, userId: string, reaction: string): Promise<void> => {
...
export const removeReactionFromMessage = async (messageId: string, userId: string): Promise<void> => {
  const messageRef = doc(db, "messages", messageId)
...
export const searchPosts = async (textQuery = "", locationQuery = "", onlyWithLocation = false): Promise<Post[]> => {
  ...
  if (onlyWithLocation) {
    ...
    snapshot = await getDocs(query(postsCollection, orderBy("createdAt", "desc")))
  }

  allPosts = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }) as Post)
  ...
  if (textQuery.trim()) {
    ...
    allPosts = allPosts.filter(
      (post) =>
        post.title?.toLowerCase().includes(lowerTextQuery) || post.content?.toLowerCase().includes(lowerTextQuery),
    )
  }
  ...
  if (locationQuery.trim()) {
    ...
    allPosts = allPosts.filter((post) => {
      ...
      return locationKeywords.some((keyword) => postLocationLower.includes(keyword))
    })
  }
...
export const getLocationSuggestions = async (partialLocation: string): Promise<string[]> => {
